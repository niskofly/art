...
$arTemplateParameters = array(
	...
	"ALLOW_NEW_PROFILE" => array(
		...
		"TYPE" => "CHECKBOX",
		"DEFAULT" => "N",
		"HIDDEN" => $arCurrentValues['ALLOW_USER_PROFILES'] !== 'Y' ? 'Y'
			: 'N',
		"PARENT" => "BASE",
	),
	"SHOW_COUPONS" => array(
...
if (!isset($arCurrentValues['SHOW_COUPONS'])
	|| $arCurrentValues['SHOW_COUPONS'] === 'Y'
) {
...
while ($arPerson = $dbPerson->GetNext()) {
	...
	if (!empty($arPers2Prop)) {
		$arTemplateParameters["PROPS_FADE_LIST_" . $arPerson["ID"]] = array(
			"NAME" => GetMessage("PROPS_FADE_LIST") . ' ('
				. $arPerson["NAME"] . ')' . '[' . $arPerson["LID"] . ']',
			"TYPE" => "LIST",
			"MULTIPLE" => "Y",
			"VALUES" => $arPers2Prop,
			"DEFAULT" => "",
