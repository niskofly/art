;(function() {
	...
	BX.Sale.BasketFilter.prototype.onFilterChange = function() {
		...
		if (
			...
			this.filterTimer = setTimeout(BX.proxy(this.enableFilterMode, this),
	...
	BX.Sale.BasketFilter.prototype.enableFilterMode = function() {
		...
		if (this.lastShownItemsHash !== foundItemsHash) {
			...
			this.component.deleteBasketItems(
					BX.util.array_values(this.component.shownItems), false);
	...
	BX.Sale.BasketFilter.prototype.searchItemMatch = function(item) {
		...
		if (BX.type.isNotEmptyString(this.currentFilter.query)) {
			...
			if (!match && this.currentFilter.query.length >= 3) {
				if (item.PRICE_FORMATED.toLowerCase().
						indexOf(this.currentFilter.query) !== -1) {
					match = 'PRICE';
				} else if (item.SUM_PRICE_FORMATED.toLowerCase().
						indexOf(this.currentFilter.query) !== -1) {
					match = 'SUM_PRICE';
				}
			}
			...
			if (!match && item.COLUMN_LIST.length) {
				for (k in item.COLUMN_LIST) {
					if (item.COLUMN_LIST.hasOwnProperty(k) &&
			...
			switch (searchMatch) {
				case 'NAME':
					entity = this.component.getEntity(
							BX(this.component.ids.item + itemData.ID), 'basket-item-name');
					if (BX.type.isDomNode(entity)) {
						entity.innerHTML = itemData.NAME.replace(
								new RegExp('(.*)(' +
										this.currentFilter.query.replace(/[.*+?^${}()|[\]\\]/g,
												'\\$&') + ')(.*)', 'gi'),
				...
				case 'PROPS':
					...
					for (i = 0; i < entity.length; i++) {
						...
						for (k in itemData.PROPS) {
							if (itemData.PROPS.hasOwnProperty(k) && itemData.PROPS[k].CODE ===
									code) {
				...
				case 'COLUMNS':
					...
					for (i = 0; i < entity.length; i++) {
						...
						for (k in itemData.COLUMN_LIST) {
							if (itemData.COLUMN_LIST.hasOwnProperty(k) &&
