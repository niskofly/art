...
if (!isset($arCurrentValues['SHOW_DISCOUNT_PERCENT'])
	...
	$arTemplateParameters['DISCOUNT_PERCENT_POSITION'] = array(
		...
		'TYPE' => 'CUSTOM',
		'JS_FILE' => CBitrixBasketComponent::getSettingsScript(
			$componentPath,
			'position'
		),
	...
	foreach ($arSkuIblockIDs as $iblockID) {
		...
		if (!empty($arOfferProps) && is_array($arOfferProps)) {
			$arTemplateParameters['OFFERS_PROPS'] = array(
				'PARENT' => 'OFFERS_PROPS',
				'NAME' => GetMessage(
					'CP_SBB_TPL_PROPERTIES_RECALCULATE_BASKET'
				),
				'TYPE' => 'LIST',
				'MULTIPLE' => 'Y',
				'SIZE' => '7',
		...
		if (!empty($arIblockIDs)) {
			...
			while ($property = $propertyIterator->fetch()) {
				...
				if (!isset($arProps[$property['CODE']])) {
					...
					$arProps[$property['CODE']]['IBLOCK_ID'][$property['IBLOCK_ID']]
						= $property['IBLOCK_ID'];

					if ($arProps[$property['CODE']]['COUNT'] < 2) {
						$arProps[$property['CODE']]['IBLOCK_TITLE'][$property['IBLOCK_ID']]
							= $arIblockNames[$property['IBLOCK_ID']];
					}
			...
			foreach ($arProps as $property) {
				...
				if ($property['COUNT'] > 1) {
					$iblockList = ($property['COUNT'] > 2 ? ' ( ... )'
						: ' (' . implode(', ', $property['IBLOCK_TITLE']) . ')');
				}

				$propList['PROPERTY_' . $property['CODE']] = $property['TITLE']
					. $iblockList;
			}
