(function() {
	...
	BX.Sale.BasketActionPool.prototype.clearLastActualQuantityPool = function(
			itemId
	) {
	...
	BX.Sale.BasketActionPool.prototype.changeQuantity = function(
			itemId,
			quantity,
			oldQuantity
	) {
		...
		if (
				(this.lastActualPool[itemId] &&
						this.lastActualPool[itemId].QUANTITY !== quantity) ||
	...
	BX.Sale.BasketActionPool.prototype.changeSku = function(
			itemId,
			props,
			oldProps
	) {
		...
		for (var id in currentPool) {
			if (currentPool.hasOwnProperty(id)) {
				for (var action in currentPool[id]) {
					if (
							currentPool[id].hasOwnProperty(action) &&
							BX.util.in_array(action, this.approvedAction)
					) {
