...
if (strlen($request->get('ORDER_ID')) > 0) {
	$APPLICATION->IncludeComponent(
		...
		array(
			...
			"PATH_TO_AUTH" => "/auth/",
			"PATH_TO_BASKET" => "/personal/order/cart_bitrix/",
	...
	if (empty($arParams['TEMPLATE_THEME'])) {
		$arParams['TEMPLATE_THEME'] = Main\ModuleManager::isModuleInstalled(
			'bitrix.eshop'
		) ? 'site' : 'blue';
	}
	...
	if ($arParams['TEMPLATE_THEME'] === 'site') {
		...
		$templateId = preg_match('/^eshop_adapt/', $templateId) ? 'eshop_adapt'
			: $templateId;
		$arParams['TEMPLATE_THEME'] = Main\Config\Option::get(
			'main',
	...
	if (!isset($arParams['DISPLAY_MODE'])
		|| !in_array(
	...
	$arParams['USE_DYNAMIC_SCROLL'] = isset($arParams['USE_DYNAMIC_SCROLL'])
	&& $arParams['USE_DYNAMIC_SCROLL'] === 'N' ? 'N' : 'Y';
	$arParams['SHOW_FILTER'] = isset($arParams['SHOW_FILTER'])
	&& $arParams['SHOW_FILTER'] === 'N' ? 'N' : 'Y';

	$arParams['PRICE_DISPLAY_MODE'] = isset($arParams['PRICE_DISPLAY_MODE'])
	&& $arParams['PRICE_DISPLAY_MODE'] === 'N' ? 'N' : 'Y';

	if (!isset($arParams['TOTAL_BLOCK_DISPLAY'])
		|| !is_array(
	...
	if (is_string($arParams['PRODUCT_BLOCKS_ORDER'])) {
		$arParams['PRODUCT_BLOCKS_ORDER'] = explode(
			',',
			$arParams['PRODUCT_BLOCKS_ORDER']
		);
	}

	$arParams['USE_PRICE_ANIMATION'] = isset($arParams['USE_PRICE_ANIMATION'])
	&& $arParams['USE_PRICE_ANIMATION'] === 'N' ? 'N' : 'Y';
	$arParams['EMPTY_BASKET_HINT_PATH'] = isset($arParams['EMPTY_BASKET_HINT_PATH'])
		? (string)$arParams['EMPTY_BASKET_HINT_PATH'] : '/';
	$arParams['USE_ENHANCED_ECOMMERCE'] = isset($arParams['USE_ENHANCED_ECOMMERCE'])
	&& $arParams['USE_ENHANCED_ECOMMERCE'] === 'Y' ? 'Y' : 'N';
	...
	if ($arParams['USE_GIFTS'] === 'Y') {
		$arParams['GIFTS_BLOCK_TITLE'] = isset($arParams['GIFTS_BLOCK_TITLE'])
			? trim((string)$arParams['GIFTS_BLOCK_TITLE'])
			: Loc::getMessage(
				'SBB_GIFTS_BLOCK_TITLE'
			);
		...
		$giftParameters = array(
			...
			'DETAIL_URL' => isset($arParams['GIFTS_DETAIL_URL'])
			...
			'SHOW_SLIDER' => isset($arParams['GIFTS_SHOW_SLIDER'])
				? $arParams['GIFTS_SHOW_SLIDER'] : '',
...
//$this->addExternalCss('/bitrix/css/main/bootstrap.css');
	$this->addExternalCss(
		SITE_TEMPLATE_PATH . '/assets/components/buttons/style.css'
	);
	$this->addExternalCss(
		SITE_TEMPLATE_PATH . '/assets/components/forms/forms_s.css'
	);

	$this->addExternalCss($templateFolder . '/themes/default/style.css');
	...
	$jsTemplates = new Main\IO\Directory(
		$documentRoot . $templateFolder . '/js-templates'
	);
	...
	$displayModeClass = $arParams['DISPLAY_MODE'] === 'compact'
	...
	if (empty($arResult['ERROR_MESSAGE'])) {
		...
		?>
		<div id="basket-root"
			 class="bx-basket bx-<?= $arParams['TEMPLATE_THEME'] ?> bx-step-opacity"
			 style="opacity: 0;">
			<?
			if (
			...
			<div class="row">
				<div class="col-xs-12">
					<div class="alert alert-warning alert-dismissable"
						 id="basket-warning" style="display: none;">
                    <span class="close"
						  data-entity="basket-items-warning-notification-close">&times;</span>
			...
			<div class="block-basket-order">
				<div class="row block-basket">
					<h2 class="bx-soa-section-title col-sm-9">
						<span class="bx-soa-section-title-count"></span>Заказ</h2>
					<div class="col-xs-12">
						<div
							...
							<div class="basket-items-list-header"
								...
								<div class="basket-items-list-header-filter">
									<a href="javascript:void(0)"
									   ...
									   data-filter="all" style="display: none;"></a>
									<a href="javascript:void(0)"
