...
$elementDeleteParams = array(
	'CONFIRM' => GetMessage(
		'CT_SPGB_TPL_ELEMENT_DELETE_CONFIRM'
	),
);
...
<div class="sale-products-gift bx-<?= $arParams['TEMPLATE_THEME'] ?>"
	...
	if (!empty($arResult['ITEMS']) && !empty($arResult['ITEM_ROWS'])) {
		...
		foreach ($arResult['ITEMS'] as &$item) {
			...
			// custom gift labels
			$item['LABEL_VALUE'] = $arParams['TEXT_LABEL_GIFT']
				?: Loc::getMessage('CT_SPGB_TPL_TEXT_LABEL_GIFT_DEFAULT');
...
<script>
	BX.message({
		...
		'CT_SPGB_CATALOG_BTN_MESSAGE_COMPARE_REDIRECT'
	)?>',
		SITE_ID: '<?=CUtil::JSEscape($component->getSiteId())?>'
	});

	var <?=$obName?> =;
	new JCSaleProductsGiftBasketComponent({
		siteId: '<?=CUtil::JSEscape($component->getSiteId())?>',
		componentPath: '<?=CUtil::JSEscape($componentPath)?>',
		deferredLoad: true,
		initiallyShowHeader: '<?=!empty($arResult['ITEM_ROWS'])?>',
		currentProductId: <?=CUtil::JSEscape(
		(int)$arResult['POTENTIAL_PRODUCT_TO_BUY']['ID']
	)?>,
