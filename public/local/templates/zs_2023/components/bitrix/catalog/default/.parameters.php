...
if ($iblockExists) {
	...
	$arTemplateParameters['LIST_PRODUCT_BLOCKS_ORDER'] = [
		...
		'TYPE' => 'CUSTOM',
		'JS_FILE' => CatalogSectionComponent::getSettingsScript(
...
if ($boolSKU) {
	...
	if ($usePropertyFeatures) {
		...
		if (!empty($arCurrentValues['DETAIL_OFFERS_PROPERTY_CODE']) && is_array(
				$arCurrentValues['DETAIL_OFFERS_PROPERTY_CODE']
			)) {
...
$arTemplateParameters['DETAIL_PRODUCT_INFO_BLOCK_ORDER'] = [
	...
	'TYPE' => 'CUSTOM',
	'JS_FILE' => CatalogElementComponent::getSettingsScript(
...
if (isset($arCurrentValues['SHOW_TOP_ELEMENTS']) && 'Y' == $arCurrentValues['SHOW_TOP_ELEMENTS']) {
	...
	if (isset($arCurrentValues['TOP_VIEW_MODE']) && $arCurrentValues['TOP_VIEW_MODE'] === 'SECTION') {
		...
		$arTemplateParameters['TOP_PRODUCT_BLOCKS_ORDER'] = [
			...
			'NAME' => GetMessage('CP_BC_TPL_PRODUCT_BLOCKS_ORDER'),
			'TYPE' => 'CUSTOM',
			'JS_FILE' => CatalogTopComponent::getSettingsScript(
				'/bitrix/components/bitrix/catalog.top',
				'dragdrop_order'
			),
			'JS_EVENT' => 'initDraggableOrderControl',
			'JS_DATA' => Json::encode([
