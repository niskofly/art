...
$iblockExists = (!empty($arCurrentValues['IBLOCK_ID'])
...
if ($iblockExists) {
	...
	while ($arProp = $rsProps->Fetch()) {
		$strPropName = '[' . $arProp['ID'] . ']' . ('' != $arProp['CODE'] ? '['
				. $arProp['CODE'] . ']' : '') . ' ' . $arProp['NAME'];
	...
	if ($usePropertyFeatures) {
		if ($iblockExists) {
			$showedProperties
				= Iblock\Model\PropertyFeature::getListPageShowPropertyCodes(
	...
	$arTemplateParameters['PRODUCT_ROW_VARIANTS'] = array(
		...
		'JS_FILE' => SaleProductsGiftBasketComponent::getSettingsScript(
			$componentPath,
			'dragdrop_add'
		),
		...
		'JS_MESSAGES' => Json::encode(
			array(
				...
				'quantity' => GetMessage(
					'CP_SPGB_TPL_SETTINGS_QUANTITY'
				),
				'quantityBigData' => GetMessage(
					'CP_SPGB_TPL_SETTINGS_QUANTITY_BIG_DATA'
				),
			)
		),
		'JS_DATA' => Json::encode(
			SaleProductsGiftBasketComponent::getTemplateVariantsMap()
		),
		'DEFAULT' => Json::encode(
	...
	if (isset($arCurrentValues['ENLARGE_PRODUCT'])
		&& $arCurrentValues['ENLARGE_PRODUCT'] === 'PROP'
	) {
	...
	$arTemplateParameters['PRODUCT_BLOCKS_ORDER'] = array(
		...
		'JS_DATA' => Json::encode(
			array(
				'price' => GetMessage(
					'CP_SPGB_TPL_PRODUCT_BLOCK_PRICE'
				),
				'quantityLimit' => GetMessage(
					'CP_SPGB_TPL_PRODUCT_BLOCK_QUANTITY_LIMIT'
				),
				'quantity' => GetMessage(
					'CP_SPGB_TPL_PRODUCT_BLOCK_QUANTITY'
				),
				'buttons' => GetMessage(
					'CP_SPGB_TPL_PRODUCT_BLOCK_BUTTONS'
				),
				'props' => GetMessage(
					'CP_SPGB_TPL_PRODUCT_BLOCK_PROPS'
				),
	...
	if ($boolSKU) {
		$arTemplateParameters['PRODUCT_DISPLAY_MODE'] = array(
			'PARENT' => 'VISUAL',
			'NAME' => GetMessage(
				'CP_SPGB_TPL_PRODUCT_DISPLAY_MODE'
			),
			'TYPE' => 'LIST',
			'MULTIPLE' => 'N',
			'ADDITIONAL_VALUES' => 'N',
	...
	if (isset($arCurrentValues['SHOW_SLIDER'])
		&& $arCurrentValues['SHOW_SLIDER'] === 'Y'
	) {
	...
	if (isset($arCurrentValues['LABEL_PROP'])
		&& !empty($arCurrentValues['LABEL_PROP'])
	) {
		if (!is_array($arCurrentValues['LABEL_PROP'])) {
			$arCurrentValues['LABEL_PROP']
				= array($arCurrentValues['LABEL_PROP']);
		}
	...
	if ($boolSKU && isset($arCurrentValues['PRODUCT_DISPLAY_MODE'])
		&& 'Y' == $arCurrentValues['PRODUCT_DISPLAY_MODE']
	) {
		...
		while ($arProp = $rsProps->Fetch()) {
			...
			if (
				...
				|| ('S' == $arProp['PROPERTY_TYPE']
					&& 'directory' == $arProp['USER_TYPE']
		...
		$arTemplateParameters['OFFER_ADD_PICT_PROP'] = array(
			'PARENT' => 'VISUAL',
			'NAME' => GetMessage(
				'CP_SPGB_TPL_OFFER_ADD_PICT_PROP'
			),
			'TYPE' => 'LIST',
			'MULTIPLE' => 'N',
			'ADDITIONAL_VALUES' => 'N',
...
if ($boolCatalog) {
	...
	if (isset($arCurrentValues['SHOW_DISCOUNT_PERCENT'])
		&& $arCurrentValues['SHOW_DISCOUNT_PERCENT'] === 'Y'
	) {
	...
	if (isset($arCurrentValues['SHOW_MAX_QUANTITY'])) {
		if ($arCurrentValues['SHOW_MAX_QUANTITY'] !== 'N') {
			$arTemplateParameters['MESS_SHOW_MAX_QUANTITY'] = array(
				...
				'TYPE' => 'STRING',
				'DEFAULT' => GetMessage(
					'CP_SPGB_TPL_MESS_SHOW_MAX_QUANTITY_DEFAULT'
				),
			);
		}
		...
		if ($arCurrentValues['SHOW_MAX_QUANTITY'] === 'M') {
			...
			$arTemplateParameters['MESS_RELATIVE_QUANTITY_MANY'] = array(
				'PARENT' => 'VISUAL',
				'NAME' => GetMessage(
					'CP_SPGB_TPL_MESS_RELATIVE_QUANTITY_MANY'
				),
				'TYPE' => 'STRING',
				'DEFAULT' => GetMessage(
					'CP_SPGB_TPL_MESS_RELATIVE_QUANTITY_MANY_DEFAULT'
				),
			);
			$arTemplateParameters['MESS_RELATIVE_QUANTITY_FEW'] = array(
				'PARENT' => 'VISUAL',
				'NAME' => GetMessage(
					'CP_SPGB_TPL_MESS_RELATIVE_QUANTITY_FEW'
				),
				'TYPE' => 'STRING',
				'DEFAULT' => GetMessage(
					'CP_SPGB_TPL_MESS_RELATIVE_QUANTITY_FEW_DEFAULT'
				),
			);
		}
	}
...
if (isset($arCurrentValues['DISPLAY_COMPARE'])
	&& $arCurrentValues['DISPLAY_COMPARE'] === 'Y'
) {
...
if (isset($arCurrentValues['USE_ENHANCED_ECOMMERCE'])
	&& $arCurrentValues['USE_ENHANCED_ECOMMERCE'] === 'Y'
) {
