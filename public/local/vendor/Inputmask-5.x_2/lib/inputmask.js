...
Inputmask.prototype = {
	...
	mask: function(elems) {
		...
		elems = elems.nodeName ?
				[elems] :
				(Array.isArray(elems) ? elems : Array.from(elems));
	...
	unmaskedvalue: function(value) {
		...
		if (this.el === undefined || value !== undefined) {
			var valueBuffer = (typeof this.opts.onBeforeMask === 'function' ?
					(this.opts.onBeforeMask.call(this, value, this.opts) || value) :
					value).split('');
			checkVal.call(this, undefined, false, false, valueBuffer);
			if (typeof this.opts.onBeforeWrite ===
					'function') this.opts.onBeforeWrite.call(this, undefined,
					getBuffer.call(this), 0, this.opts);
		}
	...
	remove: function() {
		if (this.el) {
			...
			if (cv !== getBufferTemplate.call(this).join('')) this._valueSet(cv,
					this.opts.autoUnmask); else this._valueSet('');
			//unbind all events
	...
	isValid: function(value) {
		...
		return isComplete.call(this, buffer) && value === (this.isRTL ?
				getBuffer.call(this).slice().reverse().join('') :
				getBuffer.call(this).join(''));
	...
	format: function(value, metadata) {
		this.maskset = this.maskset ||
				generateMaskSet(this.opts, this.noMasksCache);
		let valueBuffer = (typeof this.opts.onBeforeMask === 'function' ?
				(this.opts.onBeforeMask.call(this, value, this.opts) || value) :
				value).split('');
		checkVal.call(this, undefined, true, false, valueBuffer);
...
function resolveAlias(aliasStr, options, opts) {
	...
	if (aliasDefinition) {
		if (aliasDefinition.alias) resolveAlias(aliasDefinition.alias, undefined,
				opts); //alias is another alias
...
function importAttributeOptions(npt, opts, userOptions, dataAttribute) {
	function importOption(option, optionData) {
		const attrOption = dataAttribute === '' ?
				option :
	...
	if (opts.importDataAttributes === true) {
		var attrOptions = npt.getAttribute(dataAttribute), option, dataoptions,
				optionData, p;
