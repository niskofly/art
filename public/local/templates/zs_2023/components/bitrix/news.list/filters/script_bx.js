    ...
    window.JCNewsListFilter = function(params) {
        ...
        if(this.selectFilter.length){
            for(var i in this.selectFilter){
                BX.bind(this.selectFilter[i], 'change', BX.proxy(this.changeFilter, this));
            }
        }
        this.inputFilter = this.blockFilter.querySelectorAll("input");
        if(this.inputFilter.length){
            for(var i in this.inputFilter){
                BX.bind(this.inputFilter[i], 'keyup', BX.proxy(this.changeFilter, this));
            }
        }
        if (params.navParams)
        {
    ...
    window.JCNewsListFilter.prototype =
        {
            ...
            sendRequest: function (data) {
                ...
                BX.ajax({
                    url: this.ajaxPath + (document.location.href.indexOf('clear_cache=Y') !== -1 ? '?clear_cache=Y' : ''),
                    method: 'POST',
                    dataType: 'json',
                    timeout: 60,
