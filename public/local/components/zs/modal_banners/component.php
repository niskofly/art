...
/** TODO
 * убрать зависимость от DISPLAY_PROPRTIES
 * Убрать лишние настойки из компонента.
 * Убрать влияние на загодовки, крошки и мета
 * Добавить скрипть создания ИБ со всеми параметрами (пока можно просто импорт)
 */
...
//подсказка в свойству нельзя загрузить через xml по этому проверяем и если пусто то запоняем
$property = \Bitrix\Iblock\PropertyTable::getList(
	[
		'filter' => [
			'IBLOCK_ID' => $arParams['IBLOCK_ID'], // id инфоблока
			'CODE' => "SHOW_FOR_ADMIN" // id нужного свойства
		],
		'select' => [
			'ID', 'HINT'
		]
...
if(!$property['HINT']){
 $v = 'Для показа отмеченных элементов указать в GET  for_admin. Повторный показ каждый раз при обновлении';
	$propertyResult = Iblock\PropertyTable::update($property['ID'], ['HINT'=>$v]);
}
//обновление подсказки
$show_for_admin = false;
if(isset($_GET['for_admin']) && $USER->IsAdmin()){
	$show_for_admin = true;
}
...
if (empty($arParams["PAGER_PARAMS_NAME"]) || !preg_match(
		"/^[A-Za-z_][A-Za-z01-9_]*$/",
...
if ($this->startResultCache(
	...
	if($show_for_admin){
		$arFilter = array(
			...
			"IBLOCK_LID"        => SITE_ID,
			"PROPERTY_SHOW_FOR_ADMIN_VALUE" => "Y"
		);
	...
	if($show_for_admin){
		echo "<div style='padding:5px;background:red; color: white;position: fixed;top:0px;width: 100%;z-index: 1000'>Вы видите блок отладки потому, что это показ баннера настроено только для админа</div>";
	}
