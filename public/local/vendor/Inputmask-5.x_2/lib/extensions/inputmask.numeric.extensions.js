...
function findValidator(symbol, maskset) {
	...
	for (var tstNdx in maskset.tests) {
		...
		if (tstNdx >= posNdx) {
			for (var ndx = 0, ndxl = maskset.tests[tstNdx].length; ndx <
			ndxl; ndx++) {
				if ((maskset.validPositions[tstNdx] === undefined || symbol === '-') &&
...
function genMask(opts) {
	...
	if (opts.groupSeparator === opts.radixPoint && opts.digits && opts.digits !==
			'0') {
		if (opts.radixPoint === '.') {
			opts.groupSeparator = ',';
	...
	if (opts.numericInput === true && opts.__financeInput === undefined) { //finance people input style
		decimalDef = '1';
		opts.positionCaretOnClick = opts.positionCaretOnClick === 'radixFocus' ?
				'lvp' :
		...
		if (opts.radixPoint !== '' && opts.definitions[radixPointDef] ===
				undefined) {
...
function decimalValidator(chrs, maskset, pos, strict, opts) {
	...
	if (opts._radixDance && result && maskset.validPositions[radixPos] ==
			undefined) {
		return {
			insert: {
...
function checkForLeadingZeroes(buffer, opts) {
	//check leading zeros
	var numberMatches = new RegExp('(^' + (opts.negationSymbol.front !== '' ?
							escapeRegex(opts.negationSymbol.front) + '?' :
							'') + escapeRegex(opts.prefix) + ')(.*)(' + escapeRegex(opts.suffix) +
					(opts.negationSymbol.back != '' ?
	...
	(leadingzeroes[0].length > 1 || leadingzeroes[0].length > 0 &&
			leadingzeroes[0].length < number.length) ? leadingzeroes : false;
}
...
Inputmask.extendAliases({
	'numeric': {
		...
		preValidation: function(
			...
			if (radixPos !== -1 &&
					(opts._radixDance === true && isSelection === false && c ===
							opts.radixPoint && (opts.digits !== undefined &&
			...
			if (opts.__financeInput === false) {
				...
				} else if (!opts.showMaskOnHover && !opts.showMaskOnFocus &&
						!opts.digitsOptional && opts.digits > 0 &&
		...
		postValidation: function(
				buffer, pos, c, currentResult, opts, maskset, strict) {
			if (currentResult === false) return currentResult;
			if (strict) return true;
			if (opts.min !== null || opts.max !== null) {
				var unmasked = opts.onUnMask(buffer.slice().reverse().join(''),
						undefined, $.extend({}, opts, {
							unmaskAsNumber: true
						}));
		...
		onUnMask: function(maskedValue, unmaskedValue, opts) {
			...
			if (opts.unmaskAsNumber) {
				if (opts.radixPoint !== '' && processValue.indexOf(opts.radixPoint) !==
		...
		onBeforeMask: function(initialValue, opts) {
			...
			if ((typeof initialValue == 'number' || opts.inputType === 'number') &&
					radixPoint !== '') {
				initialValue = initialValue.toString().replace('.', radixPoint);
			}
			var isNagtive = initialValue.charAt(0) === '-' ||
			...
			if (radixPoint !== '') {
				...
				if (decimalPart !== '' || !opts.digitsOptional) {
					...
					if (!isNaN(parseFloat(initialValue))) {
						initialValue = (opts.roundingFN(
										parseFloat(initialValue) * digitsFactor) /
								digitsFactor).toFixed(digits);
					}
					initialValue = initialValue.toString().replace('.', radixPoint);
				}
			}
			...
			return alignDigits(initialValue.toString().split(''), digits, opts,
					forceDigits).join('');
		}
		,
					...
					case 'checkval':
						if (opts.min !== null) {
							...
							if (opts.min !== null && unmasked < opts.min) {
								return {
									refreshFromBuffer: true,
									buffer: alignDigits(opts.min.toString().
											replace('.', opts.radixPoint).
											split(''), opts.digits, opts).reverse()
								};
							}
						}
						if (buffer[buffer.length - 1] === opts.negationSymbol.front) {  //strip negation symbol on blur when value is 0
							var nmbrMtchs = new RegExp('(^' +
											...
											escapeRegex(opts.suffix) + (opts.negationSymbol.back != '' ?
													escapeRegex(opts.negationSymbol.back) + '?' :
													'') + '$)').exec(
											stripBuffer(buffer.slice(), true).reverse().join('')),
									number = nmbrMtchs ? nmbrMtchs[2] : '';
							if (number == 0) {
								result = {refreshFromBuffer: true, buffer: [0]};
							}
						} else if (opts.radixPoint !== '' && buffer[0] ===
								opts.radixPoint) { //strip radixpoint on blur when it is the latest char
						...
						if (opts.enforceDigitsOnBlur) {
							...
							result.buffer = alignDigits(bffr, opts.digits, opts, true).
									reverse();
						}
				}
			}
		...
		onKeyDown: function(e, buffer, caretPos, opts) {
			...
			if (e.ctrlKey) {
				switch (e.keyCode) {
					case keyCode.UP:
						this.inputmask.__valueSet.call(this,
			...
			if (!e.shiftKey &&
					(e.keyCode === keyCode.DELETE || e.keyCode === keyCode.BACKSPACE ||
							e.keyCode === keyCode.BACKSPACE_SAFARI) && caretPos.begin !==
					buffer.length) {
				if (buffer[e.keyCode === keyCode.DELETE ?
						caretPos.begin - 1 :
						caretPos.end] === opts.negationSymbol.front) {
					...
					if (!opts.digitsOptional) {
						if (radixPos !== -1 &&
								(caretPos.begin < radixPos || caretPos.end < radixPos ||
										(e.keyCode === keyCode.DELETE && caretPos.begin ===
												radixPos))) {
							if (caretPos.begin === caretPos.end &&
									(e.keyCode === keyCode.BACKSPACE || e.keyCode ===
											keyCode.BACKSPACE_SAFARI)) { //only adjust when not a selection
								caretPos.begin++;
							}
							bffr = buffer.slice().reverse();
							bffr.splice(bffr.length - caretPos.begin,
	...
	'indianns': { //indian numbering system
		alias: 'numeric',
		_mask: function(opts) {
			return '(' + opts.groupSeparator + '99){*|1}(' + opts.groupSeparator +
					'999){1|1}';
		},
		groupSeparator: ',',
		radixPoint: '.',
