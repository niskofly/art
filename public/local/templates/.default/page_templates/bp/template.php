...
<input type="radio" name="bp_NEW_IBLOCK_TYPE" value="Y" id="bp_NEW_IBLOCK_Y" checked onclick="bp_NewIblockTypeClick(this);"><label for="bp_NEW_IBLOCK_Y">' . GetMessage(
				"bp_wizard_iblock_new"
			) . '</label><br>
<input type="text" name="bp_NEW_IBLOCK_TYPE_TITLE" value="" style="width:90%"><br>
<input type="radio" name="bp_NEW_IBLOCK_TYPE" value="N" id="bp_NEW_IBLOCK_N" onclick="bp_NewIblockTypeClick(this);"><label for="bp_NEW_IBLOCK_N">' . GetMessage(
				"bp_wizard_iblock_select"
			) . '</label><br>
...
';
		...
		while ($arr = $rsIBlockType->Fetch()) {
			if ($ar = CIBlockType::GetByIDLang($arr["ID"], LANGUAGE_ID)) {
				...
				$s .= '<option value="' . htmlspecialchars(
						$arr["ID"]
					) . '"' . ($arr["ID"] == 'bp' ? ' selected' : '') . '>' . htmlspecialchars(
						$ar["NAME"] . " [" . $arr["ID"] . "]"
					) . '</option>';
			}
		}
