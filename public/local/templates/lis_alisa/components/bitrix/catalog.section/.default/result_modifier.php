...
if ('' != $arParams['TEMPLATE_THEME']) {
	...
	if ('site' == $arParams['TEMPLATE_THEME']) {
		...
		$arParams['TEMPLATE_THEME'] = COption::GetOptionString(
			'main',
	...
	if ('' != $arParams['TEMPLATE_THEME']) {
		if (!is_file(
			$_SERVER['DOCUMENT_ROOT'] . $this->GetFolder() . '/themes/' . $arParams['TEMPLATE_THEME'] . '/style.css'
		)) {
...
if ('Y' == $arParams['PRODUCT_DISPLAY_MODE']) {
	...
	if (empty($arParams['OFFER_TREE_PROPS']) && isset($arParams['OFFERS_CART_PROPERTIES']) && is_array(
	...
	foreach ($arResult['ITEMS'] as $key => $arItem) {
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
			if ('Y' == $arParams['PRODUCT_DISPLAY_MODE']) {
				...
				$arItem['OFFERS_PROP_CODES'] = (!empty($arUsedFields) ? base64_encode(
					serialize(array_keys($arUsedFields))
				) : '');
