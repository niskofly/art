...
var currentYear = new Date().getFullYear(),
		...
		formatCode = { //regex, valueSetter, type, displayformatter
			d: [
				...
				'day',
				Date.prototype.getDate], //Day of the month as digits; no leading zero for single-digit days.
			dd: [
				'0[1-9]|[12][0-9]|3[01]', Date.prototype.setDate, 'day', function() {
			...
			h: [
				...
				'hours',
				Date.prototype.getHours], //Hours; no leading zero for single-digit hours (12-hour clock).
			hh: [
			...
			H: [
				...
				Date.prototype.setHours,
				'hours',
				Date.prototype.getHours], //Hours; no leading zero for single-digit hours (24-hour clock).
			HH: [
				'0[0-9]|1[0-9]|2[0-3]', Date.prototype.setHours, 'hours', function() {
					return pad(Date.prototype.getHours.call(this), 2);
			...
			M: [
				...
				'minutes',
				Date.prototype.getMinutes], //Minutes; no leading zero for single-digit minutes. Uppercase M unlike CF timeFormat's m to avoid conflict with months.
			MM: [
			...
			s: [
				...
				'seconds',
				Date.prototype.getSeconds], //Seconds; no leading zero for single-digit seconds.
			ss: [
				'0[0-9]|1[0-9]|2[0-9]|3[0-9]|4[0-9]|5[0-9]',
				Date.prototype.setSeconds,
				'seconds',
			...
			l: [
				'[0-9]{3}', Date.prototype.setMilliseconds, 'milliseconds', function() {
					return pad(Date.prototype.getMilliseconds.call(this), 3);
				}], //Milliseconds. 3 digits.
			L: [
				'[0-9]{2}', Date.prototype.setMilliseconds, 'milliseconds', function() {
...
function prefillYear(dateParts, currentResult, opts) {
	if (dateParts.year !== dateParts.rawyear) {
		...
		if (enteredPart.length === 2 && enteredPart === currentYearPart) {
			const entryCurrentYear = new Date(currentYear, dateParts.month - 1,
					dateParts.day);
			if (dateParts.day == entryCurrentYear.getDate() &&
					(!opts.max || opts.max.date.getTime() >=
...
function isValidDate(dateParts, currentResult, opts) {
	if (dateParts.rawday === undefined
			|| (!isFinite(dateParts.rawday) && new Date(dateParts.date.getFullYear(),
		...
		if (dateParts.day == '29') {
			...
			if (tokenMatch.targetMatch[0] === 'yyyy' && currentResult.pos -
					tokenMatch.targetMatchIndex === 2) {
	...
	if (opts.min) {
		if (dateParts['rawyear']) {
			...
			if (rawYear < minYear) { //is out of range?
				...
				rawYear = dateParts['rawyear'].substr(0,
				...
				if (minYear <= rawYear) { //this can match
					...
					if (rawYear.length === 1 && minYear <= rawYear && rawYear <=
							maxYear && fromCheckval !== true) { //this can match
						if (tokenMatch.targetMatch[0] === 'yyyy') {
							result.insert = [
								{
		...
		if (result && dateParts['year'] && dateParts['year'] ===
				dateParts['rawyear']) {
...
function analyseMask(maskString, format, opts) {
	var dateObj = {'date': new Date(1, 0, 1)}, targetProp, mask = maskString,
			match, dateOperation;
	...
	function setValue(dateObj, value, opts) {
		...
		if (dateOperation !== undefined) {
			...
			if ((targetProp === 'day' && parseInt(datavalue) === 29) ||
					(targetProp === 'month' && parseInt(datavalue) === 2)) {
				if (parseInt(dateObj.day) === 29 && parseInt(dateObj.month) === 2 &&
			...
			if (targetProp === 'day' && parseInt(datavalue) === 0)
				datavalue = 1;
			if (targetProp === 'month' &&
					(datavalue = parseInt(datavalue), datavalue > 0))
				datavalue -= 1;
			if (targetProp === 'year' && datavalue.length < 4)
				datavalue = pad(datavalue, 4, true);
			if (datavalue !== '' && !isNaN(datavalue)) dateOperation.call(
					dateObj.date, datavalue);
		}
	}
	...
	'datetime': {
		mask: function(opts) {
			...
			opts.displayFormat = formatAlias[opts.displayFormat] ||
					opts.displayFormat || opts.inputFormat; //resolve possible formatAlias
			opts.outputFormat = formatAlias[opts.outputFormat] || opts.outputFormat ||
		...
		preValidation: function(
				buffer, pos, c, isSelection, opts, maskset, caretPos, strict) {
			if (strict) return true;
			if (isNaN(c) && buffer[pos] !== c) {
				var tokenMatch = getTokenMatch(pos, opts);
				if (tokenMatch.nextMatch && tokenMatch.nextMatch[0] === c &&
		...
		postValidation: function(
				buffer, pos, c, currentResult, opts, maskset, strict, fromCheckval) {
			...
			if (currentResult === false) { //try some shifting
				...
				if (tokenMatch.targetMatch && tokenMatch.targetMatchIndex === pos &&
						tokenMatch.targetMatch[0].length > 1 &&
					...
					if (tokenMatch.targetMatch && tokenMatch.targetMatchIndex === pos +
							1 && tokenMatch.targetMatch[0].length > 1 &&
							formatCode[tokenMatch.targetMatch[0]] !== undefined) {
						validator = formatCode[tokenMatch.targetMatch[0]][0];
					}
				}
				if (validator !== undefined) {
					if (maskset.validPositions[pos + 1] !== undefined &&
			...
			if (tokenMatch.targetMatch && tokenMatch.targetMatch[0] &&
					formatCode[tokenMatch.targetMatch[0]] !== undefined) {
				validator = formatCode[tokenMatch.targetMatch[0]][0];
				var part = buffer.slice(tokenMatch.targetMatchIndex,
						tokenMatch.targetMatchIndex + tokenMatch.targetMatch[0].length);
				if (new RegExp(validator).test(part.join('')) === false &&
