;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		bindWarningEvents: function() {
			var showItemsNode = this.getEntity(BX(this.ids.warning),
			...
			BX.bind(
					this.getEntity(BX(this.ids.warning),
		...
		scrollToFirstItem: function() {
			...
			if (BX.type.isDomNode(headerNode)) {
				var itemListTopPosition = BX.pos(
						this.getCacheNode(this.ids.itemListContainer)).top;
		...
		checkStickyHeaders: function() {
			...
			if (BX.util.in_array('top', this.params.TOTAL_BLOCK_DISPLAY)) {
				var totalBlockNode = this.getEntity(
						this.getCacheNode(this.ids.basketRoot), 'basket-total-block');
		...
		lazyLoad: function() {
			...
			if (this.getDocumentScrollTop() + window.innerHeight >=
		...
		fillTotalBlocks: function() {
			var totalNodes = this.getEntities(this.getCacheNode(this.ids.basketRoot),
					'basket-total-block');
			...
			if (totalNodes && totalNodes.length) {
				...
				if (totalTemplate) {
					...
					for (var i in totalNodes) {
						if (totalNodes.hasOwnProperty(i) &&
		...
		showWarningItemsCount: function() {
			...
			if (BX.type.isDomNode(itemCountNode)) {
				if (this.warningItems.length) {
					itemCountNode.innerHTML = this.warningItems.length + ' ' +
			...
			if (BX.type.isDomNode(itemCountNode)) {
				if (parseInt(this.result.NOT_AVAILABLE_BASKET_ITEMS_COUNT)) {
					itemCountNode.innerHTML = this.result.NOT_AVAILABLE_BASKET_ITEMS_COUNT +
							' '
							+
							this.getGoodsMessage(this.result.NOT_AVAILABLE_BASKET_ITEMS_COUNT,
		...
		bindTotalEvents: function(node) {
			if (!this.result.TOTAL_RENDER_DATA.DISABLE_CHECKOUT) {
				BX.bind(this.getEntity(node, 'basket-checkout-button'), 'click',
						BX.proxy(this.checkOutAction, this));
			}

			BX.bind(this.getEntity(node, 'basket-coupon-input'), 'change',
					BX.proxy(this.addCouponAction, this));
			BX.bind(this.getEntity(node, 'basket-coupon-input'), 'paste',
		...
		sendRequest: function(action, data) {
			...
			if (this.lastAction === 'recalculateAjax') {
				...
				data.lastAppliedDiscounts = BX.util.array_keys(
						this.result.FULL_DISCOUNT_LIST).join(',');
		...
		addPriceAnimationData: function(nodeId, start, finish, currency) {
			...
			this.priceAnimationData.int[nodeId] = (parseFloat(start) ===
					parseInt(start)) && (parseFloat(finish) === parseInt(finish));
		},
			...
			if (result.BASKET_ITEM_RENDER_DATA) {
				...
				for (i in result.BASKET_ITEM_RENDER_DATA) {
					if (result.BASKET_ITEM_RENDER_DATA.hasOwnProperty(i)) {
						...
						newData.WARNINGS = this.checkBasketItemWarnings(newData,
								result.WARNING_MESSAGE_WITH_CODE);

						if (this.items[newData.ID]) {
							if (JSON.stringify(this.items[newData.ID]) ===
					...
					if (hashMap[hash].length > 1) {
						for (var i = 0; i < hashMap[hash].length; i++) {
							...
							if (
								...
								this.items[hashMap[hash][i]].AVAILABLE_QUANTITY = this.items[hashMap[hash][i]].ALL_AVAILABLE_QUANTITY -
										otherSimilarItemsQuantity;
							}
						}
					} else if (hashMap[hash][0] &&
		...
		checkTotalAnimation: function(totalData) {
			if (this.result && this.result.TOTAL_RENDER_DATA &&
				...
				this.addPriceAnimationData(this.ids.total, totalData.PRICE,
						totalData.PRICE_NEW, totalData.CURRENCY);
			}
		...
		checkBasketItemsAnimation: function(itemData) {
			...
			if (this.items[itemId]) {
				...
				if (parseFloat(this.items[itemId].PRICE) > parseFloat(itemData.PRICE)) {
					...
					this.addPriceAnimationData(this.ids.price + itemId, itemData.PRICE,
							itemData.PRICE_NEW, itemData.CURRENCY);
				}
				...
				if (
					...
					this.addPriceAnimationData(this.ids.sumPrice + itemId,
							itemData.SUM_PRICE, itemData.SUM_PRICE_NEW, itemData.CURRENCY);
				}
			}
		...
		editGeneralWarnings: function() {
			var warningsNode = this.getEntity(this.getCacheNode(this.ids.warning),
						...
						if (this.result.WARNING_MESSAGE_WITH_CODE.hasOwnProperty(code)) {
							if (
									...
									&& generalWarningText.indexOf(
											this.result.WARNING_MESSAGE_WITH_CODE[code]) === -1
							) {
								generalWarningText += this.result.WARNING_MESSAGE_WITH_CODE[code] +
										'<br/>';
							}
						}
					}
		...
		editBasketItemWarnings: function() {
			var itemsWarningsNode = this.getEntity(
					this.getCacheNode(this.ids.warning), 'basket-item-warnings');

			if (BX.type.isDomNode(itemsWarningsNode)) {
				if (this.warningItems.length) {
					var warningCount = this.getEntity(itemsWarningsNode,
							'basket-items-warning-count');
					if (BX.type.isDomNode(warningCount)) {
						warningCount.innerHTML = this.warningItems.length + ' ' +
		...
		toggleWarningBlock: function() {
			...
			if (BX.type.isDomNode(warningNode)) {
				var generalWarningNode = this.getEntity(warningNode,
						'basket-general-warnings');
				var itemsWarningsNode = this.getEntity(warningNode,
		...
		checkBasketItemWarnings: function(itemData, warnings) {
			...
			if (BX.type.isArray(warnings[itemData.ID]) &&
					warnings[itemData.ID].length) {
				for (var i in warnings[itemData.ID]) {
					if (warnings[itemData.ID].hasOwnProperty(i) &&
			...
			if (itemWarnings.length) {
				...
				this.warningItems.splice(
						BX.util.array_search(itemData.ID, this.warningItems), 1);
			}
		...
		clearGeneralWarnings: function() {
			...
			var generalWarningNode = this.getEntity(
					this.getCacheNode(this.ids.warning), 'basket-general-warnings');
		...
		initializeBasketItems: function() {
			...
			for (var i = 0; i < this.sortedItems.length; i++) {
				if (this.useDynamicScroll && this.shownItems.length >=
		...
		createBasketItem: function(itemId) {
			...
			if (basketItemTemplate) {
				var basketItemHtml = this.renderBasketItem(basketItemTemplate,
						this.items[itemId]);
				...
				if (this.shownItems.length && sortIndex >= 0) {
					if (sortIndex <
						...
						// insert before
						BX(this.ids.item + this.shownItems[0]).
								insertAdjacentHTML('beforebegin', basketItemHtml);
						this.shownItems.unshift(itemId);
					} else if (sortIndex >
							BX.util.array_search(this.shownItems[this.shownItems.length - 1],
									this.sortedItems)) {
						// insert after
						BX(this.ids.item + this.shownItems[this.shownItems.length - 1]).
								insertAdjacentHTML('afterend', basketItemHtml);
						this.shownItems.push(itemId);
					} else {
						// insert between
						BX(this.ids.item + this.sortedItems[sortIndex + 1]).
								insertAdjacentHTML('beforebegin', basketItemHtml);
						this.shownItems.splice(sortIndex + 1, 0, itemId);
					}
				} else {
					this.getCacheNode(this.ids.itemListTable).
							insertAdjacentHTML('beforeend', basketItemHtml);
					this.shownItems.push(itemId);
				}
		...
		getItemsAfter: function() {
			...
			if (this.useDynamicScroll) {
				var lastShownItemId = this.shownItems[this.shownItems.length - 1] ||
						false;
		...
		editPostponedBasketItems: function() {
			...
			for (var i in this.postponedItems) {
				if (this.postponedItems.hasOwnProperty(i) &&
		...
		applyPriceAnimation: function() {
			...
			new BX.easing({
				duration: this.params.USE_PRICE_ANIMATION === 'Y' ?
							...
							if (!nodeCache[nodeId]) {
								if (nodeId === this.ids.total) {
									nodeCache[nodeId] = this.getEntities(
											this.getCacheNode(this.ids.basketRoot), this.ids.total);
								} else {
									var node = BX(nodeId);
				...
				complete: BX.delegate(function() {
					...
					for (nodeId in animationData.start) {
						if (animationData.start.hasOwnProperty(nodeId)) {
							formattedPrice = this.getFormatPrice(animationData.finish[nodeId],
		...
		redrawBasketItemNode: function(itemId) {
			...
			if (basketItemTemplate) {
				...
				if (oldHeight) {
					...
					if (BX.type.isDomNode(nodeAligner) && nodeAligner.clientHeight <
							oldHeight) {
		...
		bindQuantityEvents: function(node, data) {
			...
			if (block) {
				...
				BX.bind(entity, startEventName,
						BX.proxy(this.startQuantityInterval, this));
				BX.bind(entity, endEventName,
		...
		startQuantityInterval: function() {
			...
			var func = target.getAttribute('data-entity') ===
			'basket-item-quantity-minus'
		...
		isQuantityFloat: function(item) {
			return this.params.QUANTITY_FLOAT === 'Y' ||
		...
		getCorrectQuantity: function(itemData, quantity) {
			...
			var reminder = (quantity / measureRatio -
							((quantity / measureRatio).toFixed(0))).toFixed(5),
					remain;
			...
			if (measureRatio !== 0 && measureRatio !== 1) {
				remain = (quantity * this.precisionFactor) %
						(measureRatio * this.precisionFactor) / this.precisionFactor;
		...
		setQuantity: function(itemData, quantity) {
			...
			if (quantityField) {
				...
				if (parseFloat(itemData.QUANTITY) !== parseFloat(quantity)) {
					...
					this.actionPool.changeQuantity(itemData.ID, quantity,
							currentQuantity);
				}
			}
		},
			...
			if (startPrice !== finalPrice) {
				...
				new BX.easing({
					...
					step: BX.delegate(function(state) {
						...
						priceNode.innerHTML = this.getFormatPrice(state.price,
								itemData.CURRENCY);
					}, this),
					complete: BX.delegate(function() {
						var node, price;

						priceNode.innerHTML = this.getFormatPrice(finalPrice,
								itemData.CURRENCY);
		...
		changeSku: function() {
			...
			if (itemData) {
				...
				if (basketItemNode) {
					var currentSkuListNodes = this.getEntities(target.parentNode,
		...
		getSkuPropertyValues: function(basketItemNode) {
			...
			var propNodes = this.getEntities(basketItemNode, 'basket-item-sku-field',
					'.selected');
			for (var i = 0, l = propNodes.length; i < l; i++) {
				propertyValues[propNodes[i].getAttribute(
								...
								load: function() {
									...
									if (content) {
										...
										if (contentHeight > windowSize.innerHeight * ratio) {
											content.style.height = windowSize.innerHeight * ratio +
													'px';
										...
										if (contentWidth > windowSize.innerWidth * ratio) {
											content.style.width = windowSize.innerWidth * ratio +
													'px';
											content.style.height = contentHeight *
													(windowSize.innerWidth * ratio / contentWidth) + 'px';
										}
		...
		bindItemWarningEvents: function(node, data) {
			...
			BX.bind(
					this.getEntity(BX(this.ids.item + data.ID),
			...
			for (var i in data.basket) {
				if (data.basket.hasOwnProperty(i)) {
					if (i.indexOf('QUANTITY_') >= 0) {
						...
						if (this.items[itemId]) {
							itemsDiff[itemId] = parseFloat(data.basket[i]) - parseFloat(
		...
		getItemAnalyticsInfo: function(itemId, diff) {
			...
			var selectedSku = this.getEntities(BX(this.ids.item + itemId),
					'basket-item-sku-field', '.selected');
