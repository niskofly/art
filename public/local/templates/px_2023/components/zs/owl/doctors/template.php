...
$this->addExternalCss(SITE_TEMPLATE_PATH . '/assets/components/tails/tail-doctors.css');
...
if ($arResult['ITEMS']) {
	...
	<div class="doctors owl_container">
		...
		?>
		<div class="doctors__list owl-carousel" data-owl_id="<?= $id_comp ?>">
			<?php
			foreach ($arResult['ITEMS'] as $arItem) {

				?>
				<div class="owl_element"
					 style="background-image: url(<?= CFile::GetPath($arItem['PREVIEW_PICTURE']) ?>);">

					<a href="<?= $arItem['DETAIL_PAGE_URL'] ?>" class="tail-doctors"
					   style="background-image: url(<?= CFile::GetPath($arItem['PREVIEW_PICTURE']) ?>);">
						<div class="tail-doctors__bottom">
							...
							<a href="/" class="button button--def doctors__button">Записаться</a>
						</div>
					</a>
