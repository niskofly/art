...
if (CModule::IncludeModule("form")) {
	...
	if (strlen($arResult["ERROR"]) <= 0) {
		...
		if (($_POST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'] || $_POST['WEB_FORM_ID'] == $arResult['arForm']['SID']) && (strlen(
					$_REQUEST["web_form_submit"]
				) > 0 || strlen($_REQUEST["web_form_apply"]) > 0)) {
			...
			if (
				$arParams['USE_EXTENDED_ERRORS'] == 'Y' && (!is_array($arResult["FORM_ERRORS"]) || count(
		...
		*/
		...
		if ($arParams["IGNORE_CUSTOM_TEMPLATE"] == "N" && $arResult["arForm"]["USE_DEFAULT_TEMPLATE"] == "N" && strlen(
		...
		*/
		...
		foreach ($arResult["arQuestions"] as $key => $arQuestion) {
				...
				foreach ($arResult["arAnswers"][$FIELD_SID] as $key => $arAnswer) {
					...
					switch ($arAnswer["FIELD_TYPE"]) {
						...
						case "dropdown":
							if ($show_dropdown != "Y") {
								...
								if (strlen($arResult["FORM_ERROR"]) > 0) {
									...
									for ($i = 0; $i <= $c; $i++) {
										$arDropDown[$FIELD_SID]["param"][$i] = preg_replace(
											"/checked|selected/i",
											"",
						...
						case "multiselect":
							if ($show_multiselect != "Y") {
								...
								if (strlen($arResult["FORM_ERROR"]) > 0) {
									...
									for ($i = 0; $i <= $c; $i++) {
										$arResult["arMultiSelect"][$FIELD_SID]["param"][$i] = preg_replace(
											"/checked|selected/i",
											"",
						...
						case "date":
							...
							$arResult["QUESTIONS"][$FIELD_SID]["HTML_CODE"][] = $res . " (" . CSite::GetDateFormat(
									"SHORT"
								) . ")";
			...
			elseif (is_array(
					$arResult["arQuestions"][$FIELD_SID]
				) && $arResult["arQuestions"][$FIELD_SID]["ADDITIONAL"] == "Y") {
				$res = "";
			...
			$arResult["QUESTIONS"][$FIELD_SID]["HTML_CODE"] = implode(
				"<br />",
			...
			if ($arResult["QUESTIONS"][$FIELD_SID]["IS_INPUT_CAPTION_IMAGE"] == "Y") {
				...
				$arResult["QUESTIONS"][$FIELD_SID]["IMAGE"]["HTML_CODE"] = CFile::ShowImage(
					$arResult["arQuestions"][$FIELD_SID]["IMAGE_ID"]
				);
			}
		...
		$arResult["SUBMIT_BUTTON"] = "<input " . (intval(
				$arResult["F_RIGHT"]
			) < 10 ? "disabled=\"disabled\"" : "") . " type=\"submit\" name=\"web_form_submit\" value=\"" . (strlen(
				trim($arResult["arForm"]["BUTTON"])
			) <= 0 ? GetMessage("FORM_ADD") : $arResult["arForm"]["BUTTON"]) . "\" />";
		$arResult["APPLY_BUTTON"] = "<input type=\"hidden\" name=\"web_form_apply\" value=\"Y\" /><input type=\"submit\" name=\"web_form_apply\" value=\"" . GetMessage(
				"FORM_APPLY"
			) . "\" />";
		$arResult["RESET_BUTTON"] = "<input type=\"reset\" value=\"" . GetMessage("FORM_RESET") . "\" />";
		$arResult["REQUIRED_STAR"] = $arResult["REQUIRED_SIGN"];
		$arResult["CAPTCHA_IMAGE"] = "<input type=\"hidden\" name=\"captcha_sid\" value=\"" . htmlspecialcharsbx(
