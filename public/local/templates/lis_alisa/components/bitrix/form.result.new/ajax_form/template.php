...
<div class="form-container">
	...
	<div class="form-body">
		...
		?>
		<form class="ajax-form" name="<?= $arResult["arForm"]['SID'] ?>" action="<?= $templateFolder ?>/ajax.php"
			  method="POST" enctype="multipart/form-data" <?php
		if (!empty($arParams['YA_TARGET_ID'])) {
			?> form-ya_target="<?= $arParams['YA_TARGET_ID'] ?>" <?php
		}
		?>>
			<?php
			...
			foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion) {
				...
				if ($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'hidden') {
					...
					foreach ($arQuestion['STRUCTURE'] as $question) {
						...
						<input class="form-check-input <?= $inputClass ?>" type="<?= $question['FIELD_TYPE'] ?>"
							   id="field_<?= $ID ?>" name="<?= $NAME ?>"
							   value="<?= $value ?>" <?= $REQUIRED ?> <?= $question['FIELD_PARAM'] ?> />
					...
					<div class="form-group" id="field-<?= $ID ?>">
						...
						if ($FIELD_TYPE == 'radio' || $FIELD_TYPE == 'checkbox') {
								...
								foreach ($arQuestion['STRUCTURE'] as $question) {
									...
									<div class="form-check">
										<input class="form-check-input <?= $inputClass ?>"
											   ...
											   id="question_<?= $question['ID'] ?>" name="<?= $NAME ?>"
											   value="<?= $question['ID'] ?>" <?= $REQUIRED ?> <?= $question['FIELD_PARAM'] ?> />
							...
							<select class="form-control" id="field_<?= $ID ?>"
									name="<?= $NAME ?>" <?= $REQUIRED ?> <?= $FIELD_TYPE == 'multiselect' ? 'multiple' : ''; ?>>
								<?php
								foreach ($arQuestion['STRUCTURE'] as $question) {
									?>
									<option
							...
							?>
							<input class="form-control <?= $inputClass ?>" type="date" id="field_<?= $ID ?>"
								   name="<?= $NAME ?>" value=""
								   placeholder="Например 01.01.2019, или выберите в календаре ->" <?= $REQUIRED ?>>
						...
						elseif ($FIELD_TYPE == 'image' || $FIELD_TYPE == 'file') {
							...
							$NAME = 'DATA[form_' . $FIELD_TYPE . '_' . $question['ID'] . ']';
							?>
							<input class="form-control-file <?= $inputClass ?>" type="file" id="field_<?= $ID ?>"
								   name="<?= $NAME ?>" <?= $REQUIRED ?>>
							<?php
							...
							$NAME = 'DATA[form_' . $FIELD_TYPE . '_' . $question['ID'] . ']';
							?>
							<input class="form-control <?= $inputClass ?>" type="<?= $FIELD_TYPE ?>"
								   id="field_<?= $ID ?>" name="<?= $NAME ?>" aria-describedby="emailHelp"
								   placeholder="<?= $placeholder ?>" <?= $REQUIRED ?> <?= $question['FIELD_PARAM'] ?>>
							<?php
						} else {
							$question = $arQuestion['STRUCTURE'][0];
							$NAME = 'DATA[form_' . $FIELD_TYPE . '_' . $question['ID'] . ']';
			...
			<div class="ajax-form__footer">
				<div class="ajax-form__buttons">
					<input class="button button--dark" type="submit" name="web_form_submit" value="<?= $buttonText ?>"/>
				</div>
				...
				<div class="agreement">
					...
					Нажимая на кнопку «отправить», Вы принимаете условия <a target="_blank"
																			href="<?= $arParams['AGREEMENT_LINK'] ?>">Пользовательского
						соглашения</a>
				</div>
