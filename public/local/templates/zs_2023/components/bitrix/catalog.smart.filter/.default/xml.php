...
>
	<title><?
		echo $arResult["SECTION_TITLE"] ?></title>
	<description><?
		echo $arResult["SECTION_DESCRIPTION"] ?></description>
	<rootUrl><?
		echo CHTTP::urn2uri("/"); ?></rootUrl>
	<resource>
		<fixed name="<?
		...
		foreach ($arResult["ITEMS"] as $arItem): ?>
			...
			if ($priceCounter && isset($arItem["PRICE"])): ?>
				...
				if (isset($arItem["VALUES"]["MIN"]["VALUE"]) && isset($arItem["VALUES"]["MAX"]["VALUE"])): ?>
					...
					$totalCounter--; ?>
					<rangeFilter min="<?
					echo floor($arItem["VALUES"]["MIN"]["VALUE"]) ?>"
								 max="<?
								 echo ceil($arItem["VALUES"]["MAX"]["VALUE"]) ?>"
								 step="1" <?
					if (count($arItem["CURRENCIES"]) == 1) {
						echo ' unit="' . current($arItem["CURRENCIES"]) . '"';
					} ?>>
						<description caption="<?
						echo $arItem["NAME"] ?>">
							<setParameter name="<?
			...
			elseif ($totalCounter && !empty($arItem["VALUES"])):; ?>
				...
				<dropDown>
					...
					foreach ($arItem["VALUES"] as $val => $ar): ?>
						<dropDownValue key="<?
						echo $ar["HTML_VALUE_ALT"] ?>" caption="<?
						echo $ar["VALUE"]; ?>"/>
					<?
					endforeach; ?>
				</dropDown>
